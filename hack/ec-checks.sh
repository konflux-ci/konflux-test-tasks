...
function build_container_task_name {
  local pipeline=$1
  yq '.spec.tasks[] | select(.name == "build-container") .taskRef.name' < "$pipeline" | tr -d '"'
}
...
function copy_all_task_versions {
  ...
  done < <(find task/"${task}"/*/ -maxdepth 0 -type d)
}
...
function build_tasks_dir {
  ...
  local generated_pipelines_dir
  generated_pipelines_dir=$(mktemp -d)
