...
- Files in `task/*-oci-ta/` directories (e.g., `pickle-scan-oci-ta.yaml`) are auto-generated by the trusted-artifacts task generator from `recipe.yaml` files. Do not suggest style or formatting changes to these files — modify the base task in `task/<name>/` or the `recipe.yaml` instead, then regenerate with `hack/generate-ta-tasks.sh`.
